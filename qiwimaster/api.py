...
from . import QIWIAPIError, TokenError, NumberError
...
class QIWIApi(object):
    ...
    def __init__(
        ...
        if not token and not auth_key:
            ...
            raise NumberError("Invalid Number! Please, enter the Qiwi Wallet number.")
    ...
    async def balance(self, only: bool = False) -> Union[float, List[float]]:
        ...
        if not self.token or not self.phone:
            raise TokenError("Invalid Token or phone! You can get it here: https://qiwi.com/api")
    ...
    async def full_balance(self) -> List[Dict[str, Union[str, float]]]:
        ...
        response = await self.session.get('https://edge.qiwi.com/funding-sources/v1/accounts/current')
    ...
    def pay(
        ...
        if not number:
            raise NumberError("Invalid Number! Please, enter the number to which you want to transfer money.")
        
        elif not amount:
            raise QIWIAPIError("Invalid Amount! Please, enter the amount you want to transfer.")
    ...
    def check(self, bill_id: int) -> dict:
        ...
        self.session.headers["Authorization"] = f"Bearer {self.auth_key}"
        return self.request("GET", f"https://api.qiwi.com/partner/bill/v1/bills/" + bill_id)

    def cancel_payment(bill_id: int) -> dict:
        """
        Отменяет счёт.
        ...
        self.session.headers["Authorization"] = f"Bearer {self.auth_key}"
        return self.request("POST", f"https://api.qiwi.com/checkout-api/api/bill/reject", {"id": bill_id})
